		...
		<div class="col-md-6" style='display:none'>
			<div class="card">
				...
				<div class="content row" style='padding:10px;min-height:300px'>
					...
					<div id='photo_div'>
<?php
						$directory = "library/general/images";
						$images = glob($directory . "/*.*");
						$imgsize = 0;
						foreach($images as $image)
						{
							echo "<div style='margin-top:20px' class='col-md-4'><img style='cursor:pointer;object-fit: cover;width:100%;height:200px' src='{$image}' class='img-rounded delete_photo'></div>";
						  $imgsize++;
						}
?>					
					</div>
		...
		<div class="col-md-12">
			<div class="card">
				...
				<div class="content row" style='padding:10px;min-height:300px'>
					...
					<div id='files_div'>				
<?php
						$directory = "library/general/files";
						$images = glob($directory . "/*.*");
						$imgsize = 0;
						...
						foreach($images as $image)
						{
							...
							echo "<div style='margin-top:20px' class='col-md-2'><img style='cursor:pointer;object-fit: cover;width:100%;height:150px' src='$typefile' src2='{$filename[3]}' type='general' class='img-rounded delete_file'><text class='changetitle' src='$image'>{$filename[3]}<br><b>General</b></text><br><a class='noblock' target='_blank' href='library/general/files/{$filename[3]}'>Ver</a></div>";
						  $imgsize++;
						}
						$directory = "library/teledeteccion/files";
						$images = glob($directory . "/*.*");
						...
						foreach($images as $image)
						{
							...
							$mime = get_mime_type($image);
							$typefile = get_mime_image($mime);							
							echo "<div style='margin-top:20px' class='col-md-2'><img style='cursor:pointer;object-fit: cover;width:100%;height:150px' src='$typefile' src2='{$filename[3]}' type='teledeteccion' class='img-rounded delete_file'><text class='changetitle' src='$image'>{$filename[3]}<br><b>Teledeteccion</b></text><br><a class='noblock' target='_blank' href='library/teledeteccion/files/{$filename[3]}'>Ver</a></div>";
						  $imgsize++;
						}
						
						$directory = "library/metagenomica/files";
						$images = glob($directory . "/*.*");
						...
						foreach($images as $image)
						{
							...
							$mime = get_mime_type($image);
							$typefile = get_mime_image($mime);							
							echo "<div style='margin-top:20px' class='col-md-2'><img style='cursor:pointer;object-fit: cover;width:100%;height:150px' src='$typefile' src2='{$filename[3]}' type='metagenomica' class='img-rounded delete_file'><text class='changetitle' src='$image'>{$filename[3]}<br><b>Metagenomica</b></text><br><a class='noblock' target='_blank' href='library/metagenomica/files/{$filename[3]}'>Ver</a></div>";
						  $imgsize++;
						}
						...
						foreach($images as $image)
						{
							...
							$mime = get_mime_type($image);
							$typefile = get_mime_image($mime);							
							echo "<div style='margin-top:20px' class='col-md-2'><img style='cursor:pointer;object-fit: cover;width:100%;height:150px' src='$typefile' src2='{$filename[3]}' type='monitoreo' class='img-rounded delete_file'><text class='changetitle' src='$image'>{$filename[3]}<br><b>Estacion de Monitoreo</b></text><br><a class='noblock' target='_blank' href='library/monitoreo/files/{$filename[3]}'>Ver</a></div>";
						  $imgsize++;
						}						
?>					
					</div>
...
<script type="text/javascript">
	jQuery(document).ready(function() {	
		...
		$('#file_upload_images').uploadifive({
			'formData'  : {'type':'general','subtype':'images'},
			'fileSizeLimit': "2MB",
			'queueID'          : 'queue',
			...
			'onUploadComplete' : function(data) {
					$.ajax({
						...
						data: {'type':'general','subtype':'images'},
						cache: false,
						success: function(output){
							$('#noimages').hide();
							$('#photo_div').empty();
		...
		$('#file_upload_files').uploadifive({
			'formData'  : {'type':'general','subtype':'files'},
			'fileSizeLimit': "2MB",
			'queueID'          : 'queue_files',
			'removeCompleted' : true, 
			'buttonText'   : 'Agregar',
			'onUploadComplete' : function(data) {
					$.ajax({
						type: "POST",
						url: "jquery/getImages-general.php",
						data: {'type':'general','subtype':'files'},
						cache: false,
						success: function(output){
							$('#nofiles').hide();
							$('#files_div').empty();
			...
			function(vari){
				if (vari.value) {
					$.ajax({
						...
						data: {'type':'general','subtype':'images','imagelink':imagelink},
						cache: false,
						success: function(output){
		...
		$(document).on('click','.delete_file', function() {
			...
			var imagelink = "library/" + type + "/files/" + $(this).attr('src2');
			swal({
			...
			function(vari){
				if (vari.value) {
					$.ajax({
						type: "POST",
						url: "jquery/deleteImage-general.php",
						data: {'type':'general','subtype':'files','imagelink':imagelink},
						cache: false,
						success: function(output){
							$('#nofiles').hide();
							$('#files_div').empty();
