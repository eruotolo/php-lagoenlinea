		...
		<div class="col-md-6" style='display:none'>
			<div class="card">
				...
				<div class="content row" style='padding:10px;min-height:300px'>
					...
					<div id='photo_div'>
<?php
						$directory = "library/teledeteccion/images";
...
<script type="text/javascript">
	jQuery(document).ready(function() {	
		...
		$('#file_upload_images').uploadifive({
			'formData'  : {'type':'teledeteccion','subtype':'images'},
			'queueID'          : 'queue',
			...
			'onUploadComplete' : function(data) {
					$.ajax({
						...
						data: {'type':'teledeteccion','subtype':'images'},
						cache: false,
						success: function(output){
							$('#noimages').hide();
							$('#photo_div').empty();
		...
		$('#file_upload_files').uploadifive({
			'formData'  : {'type':'teledeteccion','subtype':'files'},
			'queueID'          : 'queue_files',
			'removeCompleted' : true, 
			'buttonText'   : 'Agregar',
			'onUploadComplete' : function(data) {
					$.ajax({
						type: "POST",
						url: "jquery/getImages.php",
						data: {'type':'teledeteccion','subtype':'files'},
						cache: false,
						success: function(output){
							$('#nofiles').hide();
							$('#files_div').empty();
			...
			function(vari){
				if (vari.value) {
					$.ajax({
						...
						data: {'type':'teledeteccion','subtype':'images','imagelink':imagelink},
						cache: false,
						success: function(output){
		...
		$(document).on('click','.delete_file', function() {
			var imagelink = "library/teledeteccion/files/" + $(this).attr('src2');
			swal({
			...
			function(vari){
				if (vari.value) {
					$.ajax({
						type: "POST",
						url: "jquery/deleteImage.php",
						data: {'type':'teledeteccion','subtype':'files','imagelink':imagelink},
						cache: false,
						success: function(output){
